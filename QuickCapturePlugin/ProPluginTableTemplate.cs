...
namespace QuickCapturePlugin {
	...
	public class ProPluginTableTemplate : PluginTableTemplate, IDisposable, IPluginRowProvider {
		...
		private void Open() {
			...
			using (SQLiteCommand cmd = _dbConn.CreateCommand()) {
				...
				while (reader.Read()) {
					...
					// TODO Assumption: okay to skip a feature error row with no data in Feature field
					if (sFeat.Length > 0) {
						// TODO Assumption: if feature data exists, it will be well-formed and correct
		...
		private Geometry BuildFeatureGeometry(string sGeom) {
			// TODO Assumption: all QuickCapture features have geometries
